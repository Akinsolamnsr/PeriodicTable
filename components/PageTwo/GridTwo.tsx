    ...
    <div className={`${Slide && !status?"":`${state.margin?"":`${status?"":"-ml-[6rem]"}`}`} inline-flex flex-col mt-8 font-bold `}>
        ...
        {array.map((x:any,i:any,arr:any)=>{
            ...
            if(Arr.includes(x[1])){
                ...
                return(
                    <div className={`${trans&&Slide && !status?"translate-x-[52.5rem]":""}  ${status?"w-[6rem] h-[7rem] ml-1 mt-1":"w-[3.5rem] h-[4rem] ml-1 mt-1"} text-center  ${Stat && status?"hidden ":""}`} >{<BatchTwo {...x[2]} />}</div>
                      )
            }
